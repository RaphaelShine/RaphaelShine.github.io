...
## 시작하며
⠀이번 포스트에서는 Jekyll이 다른 모든 코드들을 계산하여 결과물로 내놓는, 바로 **HTML**에 대해 배울 것입니다. HTML은 웹 개발 하면 나오는 세 가지(HTML, CSS, JS) 중 첫번째입니다. HTML을 작성하면, 적힌 그대로 웹 페이지가 변화 없이 나타나게 됩니다. 이를 <span style='color:lightblue'>**정적**</span>이라고 합니다. 반대로 <span style='color:pink'>**동적**</span>인 것은 변수에 따라 페이지 구성이 변하는 것으로, 여러분이 입력한 아이디에 따라 웹 페이지에 보이는 여러분의 프로필이 달라지는 것을 예로 들 수 있겠습니다. 따라서 우리는 변수를 처리하기 위해 <span style='color:lightblue'>정적인 **HTML**</span>위에 <span style='color:pink'>동적인 **Liquid**</span>라는 언어를 끼얹을 겁니다. Liquid는 HTML을 보조해 주기 위한 언어로, 변수처리를 하긴 하지만 단순히 HTML을 만드는 데만 쓰여 내용이 별로 없습니다. 그러니 만만하게 보고 아무렇게나 따라오셔도 되겠습니다.

## HTML
⠀지금은 블로그 만드는 데 꼭 필요한 것만 설명하겠습니다.
...
⠀tag는 글 묶는 법 정도로 생각하면 되겠고, attribute는 tag에 붙은 속성으로, 세부사항을 결정합니다. 같은 tag로 묶인 서로 다른 두 글을 구별하기 위해 class라는 attribute를 붙이거나 색을 입히기 위해 style을 붙이는 식입니다.
...
## Liquid
⠀Liquid는 액체라는 뜻인 거 여러분 모두 아시리라 생각합니다. HTML은 구성이 변수의 변화에 따라 흐르지 않지만 Liquid는 유동적으로 변수의 변화에 따라 HTML을 작성합니다. 즉, Liquid가 하는 일이 무엇인지 이름에서부터 드러나는 것입니다(뇌피셜). 코딩의 코자도 모른다고 간주하고 설명하겠습니다.
...
⠀이제 중요한 tag들을 소개할 예정인데, 이건 어려우면 건너뛰고 나중에 필요할 때 사전 펼치듯 보면 됩니다.

### &#123;% assign %&#125;  
⠀Liquid는 숫자, 문자열, 배열, object, nil을 변수에 담을 수 있습니다. nil은 값이 존재하지 않음을 뜻합니다.
```liquid
{% raw %}{% assign One = 1 %}
{{ One }}은 1입니다
...
⠀Markdown에서는 `\`를 이용해서도 작성이 가능합니다. Liquid의 중괄호엔 적용되지 못하지만 어차피 이중 중괄호나 중괄호 퍼센트를 쓸 일은 없어 보입니다. 그럼에도 그런 것까지 쓰고싶다면 [고수 포스트](/blog/blog-7-hard-html-and-liquid/)를 확인하십시오.
```markdown
\<strong>안녕?!</strong>
```
\<strong>안녕?!</strong>
