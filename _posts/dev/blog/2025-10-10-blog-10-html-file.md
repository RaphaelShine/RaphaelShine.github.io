---
title: "[Blog]10. Jekyll의 HTML 파일 정리 (Minimal-Mistakes)"
excerpt: "Jekyll의 HTML 파일들(includes, layouts)의 사용 방법(포스트 이동버튼 카테고리 기준 수정 등)을 알아보고 Minimal-Mistakes를 기준으로 구체적으로 정리한다."
...
⠀HTML은 딱히 사용 방법이라고 할 게 없고, 알아서 코딩하면 될 것 같습니다. 그러므로 이 포스트는 사전처럼 만들어 둘 테니, 필요한 기능을 찾거나 파일이 뭐하는 파일인지 알고 싶을 때 찾으시면 되겠습니다. 코드가 필요하면 제 리포지토리를 이용하십시오.

⠀참고로 저는 Minimal-Mistakes를 fork해왔기 때문에 Minimal-Mistakes의 파일 기능과 그것을 어떻게 수정했는지를 중심으로 설명합니다. 이 템플릿을 쓰지 않았더라도 원하는 부분 찾아서 가져가실 수 있습니다. 블로그의 파일은 대부분이 HTML이라 이 포스트가 상당히 길어질 예정입니다. 알 필요 없는 부분은 넘어가겠습니다.
...
## `page__hero.html`, `page__meta.html`
⠀`page__meta.html`은 수정했습니다.
...
## `single.html`
⠀모든 포스트가 이 문서의 틀 위에 있습니다. `_config.yml`에서 설정되어 있는 `site.defaults.values.layout`이 single로 되어있기 때문입니다.
## `field.html`
