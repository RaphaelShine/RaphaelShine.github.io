...
## 댓글 달기
⠀댓글 기능을 만드는 건 스스로 코드를 만들어 하기에는 조금 복잡하기 때문에 보통은 댓글 기능을 제공해주는 플랫폼을 이용합니다. 쉽고 편한 방법을 원하신다면 utterances나 disqus같은 댓글 플랫폼을 찾아보시면 되겠습니다. 물론 이 블로그의 코드를 가져가도 됩니다.

⠀이 블로그의 댓글 코드를 쓰거나, 만약 어려운 길이지만 스스로 댓글 시스템을 만들고 싶다면, 자세한 건 [댓글 만드는 포스트](/blog/blog-14-creating-comment-system/)에서 확인하십시오.
