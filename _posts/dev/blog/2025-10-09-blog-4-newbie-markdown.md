...
excerpt: "블로그를 위해 Markdown(마크다운)의 기본 작성법과 팁."

sort_key : 4
categories:
  - Blog
tags:
  - [Blog-뼈대, 프런트엔드, Blog-날먹, Blog-초보]
...
### 코드블록
⠀글을 \`(백틱)으로 묶으면 `이렇게`됩니다. 다른 특수문자들이 실행되지 않게 해 주는 기능도 있습니다. `<span style='color:red'>이렇게 HTML 문법도 무시합니다.</span>`
...
|:---:|---:|:---|---|
|`:`에 따라 정렬.|`-`는 3개 이상이기만 하면 됨.|내용내용내용|내용내용내용|
|내용|내용|내용|내용|
```
...
⠀!\[글](이미지)와 같이 적습니다. 위 예시와 같이 디렉토리로 넣을 수도 있고, 이미지 링크를 넣어도 됩니다. 저 중괄호는 attribute입니다.
## Attribute 달기
⠀링크와 이미지에는 attribute를 달 수 있습니다. HTML 설명할 때 졸았거나 건너 뛰었거나 기억이 안 날 여러분을 위해 설명드리자면, attribute라는 건 한국어로 속성이라고 하고, 그냥 글에 다는 첨가물입니다. 링크를 눌러 타 사이트로 이동할 때 보안을 지켜주는 기능이나 이미지 크기와 정렬을 결정하는 기능을 합니다. 
...
⠀이 attribute는 링크를 새 탭으로 열도록 만듭니다. 타 사이트로 이동할 때는 이 attribute를 복사해서 붙여넣으면 좋습니다.
